...
public class HttpRequest implements Request {
    ...
    /**
     * Serialized Body for the http request.
     *
     * @return The request serialized body as {@link String}.
     */
    public String getSerializedBody() {
        return body instanceof String
                ? body.toString()
                : ApiHelper.trySerialize(body);
    }

    public static HttpRequest fromHttpServletRequest(
        ...
        parameterMap.forEach((k, v) -> {
            if (v != null && v.length == 1) {
                queryParameters.put(k, v[0]);
            } else if (v != null) {
                queryParameters.put(k, Arrays.asList(v));
            }
        });
    ...
    public static HttpRequest fromSpringRequest(
            Map<String, List<String>> headersMap,
            String method,
            Object body,
            String url) {
        ...
        Headers headers = new Headers(headersMap);
        // Query parameters extraction (if URL contains ?)
        Map<String, Object> queryParameters = new HashMap<>();
        String baseUrl = url;
        if (url.contains("?")) {
        ...
        // Return HttpRequest
        return new HttpRequest(httpMethod, new StringBuilder(baseUrl), headers, queryParameters, body);
    }
