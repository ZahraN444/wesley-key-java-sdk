    ...
    public ResponseEntity<String> receiveEvent(
        ...
        String result = handler.verifyAndParseEventAsync(httpRequest).thenApply(webhooksParsingResult ->
            webhooksParsingResult.matchSome(new WebhooksParsingResult.SomeCases<String>() {
                @Override
                public String primitiveCollectionEvent(PrimitiveCollectionEvent primitiveCollectionEvent) {
                    return MessageFormat.format("PrimitiveCollectionEvent event received {0}", primitiveCollectionEvent.toString());
                }
